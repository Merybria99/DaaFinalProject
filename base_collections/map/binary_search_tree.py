...
class TreeMap(LinkedBinaryTree, MapBase):
    ...
    """
        ...
        while parent is not None and self.left(parent) == walk:
            walk = parent
            parent = self.parent(walk)
            return parent
    ...
    def countRange(self, start, stop):
        """Find the numer of elements between start and stop in natural position"""
        self._validate(start)
        self._validate(stop)
        walk = start
        count = 0
        """check if the given positions are valid start<stop"""
        if not start.key() < stop.key():
            raise ValueError("start's key is not less than stop's key")
